          ...
          <div class="col-md-12">
            ...
            <form method="post" class="search-jobs-form">
              <div class="row mb-5">
                ...
                <div class="col-12 col-sm-6 col-md-6 col-lg-3 mb-4 mb-lg-0">
                  <select class="selectpicker" data-size="3" data-style="btn-white btn-lg" data-width="100%" data-live-search="true" title="Select Region" name="searchlocation"  name="searchlocation">
                  <?php 
      ...
      <div class="container">
        <ul class="job-listings mb-5">
          ...
          { 
            ...
            // Initialize the AND condition for job status
            $and = ' job_status = "Active"';
    
            // Check if searchjob is not empty and add the condition
            if(!empty($_POST['searchjob'])){
                $searchJob = mysqli_real_escape_string($connect, $_POST['searchjob']);
                trim($searchJob);
                $and .= " AND LOWER(Job_Post_Title) LIKE LOWER('%$searchJob%')";
                
                echo "User Input: " . $searchJob;
            }
    
            // Check if searchlocation is not empty and add the condition
            if(!empty($_POST['searchlocation'])){
                $searchLocation = mysqli_real_escape_string($connect, $_POST['searchlocation']);
                $and .= " AND Job_Post_Location LIKE '%$searchLocation%'";
            }
            if(!empty($_POST['searchtype'])){
              $searchType = mysqli_real_escape_string($connect, $_POST['searchtype']);
              $and .= " AND Job_Post_Type = '$searchType'";
            }
            ...
            // Append the AND condition to the query
            $query .= $where . $and;
            //count total row foe the search result
