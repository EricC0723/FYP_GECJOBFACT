...
<?php
	...
	?>
		...
		$sql = mysqli_query($connect, "SELECT * FROM admins where Email = '$email'");
		...
		if($count > 0)
		{
			...
			if($status=="Active" && $password == $adminpassword){
				$_SESSION['Admin_ID'] = $fetch['AdminID'];
				$_SESSION['First_Name'] = $fetch['FirstName'];
                $_SESSION['Last_Name'] = $fetch['LastName'];
				$_SESSION['profile'] = $fetch['AdminPicture'];
				$_SESSION['AdminType'] = $fetch['AdminType'];
				?>
				<script>
					console.log("login");
					swal({
