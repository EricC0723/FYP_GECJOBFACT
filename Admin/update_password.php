				...
				<div class="col-md-6">
					<div class="login-box bg-white box-shadow border-radius-10">
						<div class="login-title">
							<h2 class="text-center text-primary">Set Up Password</h2>
						</div>
						<h6 class="mb-20">Enter your password, confirm and submit</h6>
						<form method="post">
							...
							<div class="row align-items-center">
								<div class="col-5">
									<div class="input-group mb-0">
										<input class="btn btn-primary btn-lg btn-block" type="submit" value="Submit" name="reset">
									</div>
								</div>
							</div>
						</form>
    ...
    {   $adminID = $_GET['admin_id'];
        if(isset($_POST["reset"])){
            ...
            elseif (!preg_match('/^(?=.*[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]).*$/', $psw)) {
                ?>
                <script>
                    displayError($('input[name="comfirmpw"]'), 'Password must contain at least one special characters');
                </script>
                <?php
            }
            elseif ($psw != $c_psw) {
            ...
            $result = mysqli_query($connect, "UPDATE admins SET Password='$psw' WHERE AdminID='$adminID'");
