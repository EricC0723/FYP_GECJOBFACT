...
<div style="width:100%;padding-top:32px;">
    <div style="flex-direction:row;display:flex;justify-content:space-between;align-items:center;">
        ...
        $sql = "SELECT COUNT(*) as total FROM job_post WHERE CompanyID = $CompanyID AND job_status = 'Blocked' AND Job_Post_Title LIKE '%$searchTerm%' AND Job_isDeleted = '0'";
        ...
        <div>
            <form id="blockedForm" method="GET" action="">
...
<div style="width: 100%;margin: auto;height: 100%;padding-top:12px;">
    ...
    if (mysqli_num_rows($result) > 0) {
            ...
            <tbody><tr style="border-top: 4px solid #f5f6f8;height: 80px">
                            <td>
                                <div class="td_title"><span class="blocked-box">
                                    <span class="blocked-text">' . htmlspecialchars($row['job_status']) . '</span></span>
                                </div>
                            </td>
                            <td>
                                    <div class="td_title">
        ...
        $sql_total = "SELECT COUNT(*) FROM job_post WHERE CompanyID = $CompanyID AND job_status = 'Blocked' AND Job_Post_Title LIKE '%$searchTerm%' AND Job_isDeleted = '0' ORDER BY AdStartDate DESC";
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            ...
            if ($total_pages < 10) {
                $pagLink .= "<button class='page-button blocked" . ($i == $currentPage ? " current-page" : "") . "' data-page-number='" . $i . "'>" . $i . "</button>";
            } else {
