...
<div style="width: 100%;margin: auto;height: 100%;padding-top:12px;">
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql_total = "SELECT COUNT(*) FROM job_post WHERE CompanyID = $CompanyID AND job_status = 'Closed' AND Job_Post_Title LIKE '%$searchTerm%' AND Job_isDeleted = '0' ORDER BY AdStartDate DESC";
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            ...
            if ($total_pages < 10) {
                $pagLink .= "<button class='page-button closed" . ($i == $currentPage ? " current-page" : "") . "' data-page-number='" . $i . "'>" . $i . "</button>";
            } else {
